...
import { SORT_FIELDS, sortCaptains } from "./buildCaptains";
  ...
  return (
    <td className={`px-1 py-1 text-center border-b border-gray-100 ${bg}`}>
      <div className="flex flex-col items-center leading-none">
        <span className="text-[12px] font-extrabold tabular-nums text-black">
          {obtenidos} / {posibles}
        </span>
        <span className="mt-0.5 text-[12px] tabular-nums text-slate-600">
          {efec}%
        </span>
      </div>
    </td>
...
const CaptainsTable = ({
  ...
  // campo de la condición según el título
  const condKey =
    title === "Local"
      ? "local"
      : title === "Visitante"
  ...
  const sorted = sortCaptains(list, sortField, sortDir, condKey);
  const getRow = (cap) => (condKey ? cap[condKey] : cap);
          ...
          <table className="w-max text-xs border-separate border-spacing-px bg-white">
            ...
            <tbody>
              {sorted.map((cap, i) => {
                ...
                return (
                  <tr
                    key={`${cap.captain}-${cap.club}-${i}`}
                    className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}
                  >
                    <td className="px-1 py-1 border-b border-gray-100 max-w-[120px]">
