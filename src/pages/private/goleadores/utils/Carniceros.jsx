...
const Carniceros = ({ matches = [] }) => {
  const Carniceros = useMemo(() => {
    const map = new Map();

    (Array.isArray(matches) ? matches : []).forEach((m) => {
      const lista = Array.isArray(m?.goleadoresRivales)
        ? m.goleadoresRivales
        : [];
      ...
      lista.forEach((g) => {
        ...
        const club = (g.club || g.equipo || g.rival || "").trim(); // probamos varios nombres posibles
        if (!nombre) return;

        const key = `${nombre}||${club}`;
    ...
    return Array.from(map.entries())
      .map(([key, expulsiones]) => {
        ...
        return { nombre, club, expulsiones };
      })
      .sort(
        (a, b) =>
